...
```{r + Shitty try with only uniform}

brm_out1 <- brm(
  pitchSD_s  ~ 1 + diagnosis + (1 | studynr), 
  prior = set_prior("uniform(0,1000)", class = "b"),
  data = pitch_s, 
  cores = 4
)
...
#What would this say?
get_prior(pitchSD_s  ~ 1 + diagnosis + (1 | studynr),
          data = pitch_s)
...
brm_mod_all <- brm(
 pitchSD_s ~ 1 + diagnosis + (1 | studynr) + (1 | ID_unique) + (1 | trial), 
  prior = c(
    prior(normal(0, 0.1), class = "Intercept"),
    prior(cauchy(0, 0.1), class = "sd", coef = "Intercept", group = studynr),
    prior(cauchy(0, 0.1), class = "sd", coef = "Intercept", group = ID_unique),
    prior(cauchy(0, 0.1), class = "sd", coef = "Intercept", group = trial),
...
#Unlike the way rethinking’s extract.samples() yields a list, brms’s posterior_samples() returns a data frame.
post1 <- posterior_samples(brm_mod_all)
#str(post1)
#pairs(post1)
...
post <- posterior_samples(brm_mod, add_chain = T)
...
```{r + model estimates from meta model}
...
```{r + plotting the models against each other}
plot.brm.all <- f %>%
  ggplot(aes(x = Estimate)) +
